...
def my_main(spark,
    ...
    # TO BE COMPLETED
    # ---------------------------------------
    r1DF = inputDF.select(f.col("*")).where((f.col("atStop") != 0) & (f.col("vehicleID") == vehicle_id) &(f.to_date(f.lit(day_picked),"yyyy-MM-dd") == f.to_date(inputDF["date"],"yyyy-MM-dd HH:mm:ss")) )

    rxDF= r1DF.orderBy(f.to_timestamp(r1DF["date"]))

    #ryDF = rxDF.dropDuplicates(["busLineID","closerStopID"])

    r2DF = rxDF.select(f.col("busLineID").alias("lineID"),f.col("closerStopID").alias("stationID"),f.col("delay"),f.col("date"))\
               .withColumn("arrivalTime",f.date_format(r1DF["date"],"HH:mm:ss"))
    r3DF = r2DF.select(f.col("lineID"),f.col("stationID"),f.col("arrivalTime"),f.col("delay"))\
               .orderBy(f.col("arrivalTime"))
    r4DF = r3DF.withColumn("onTime",f.when(f.abs(r3DF["delay"])>=delay_limit,0)\
               .otherwise(1))
    r5DF = r4DF.select(f.col("lineID"),f.col("stationID"),f.col("arrivalTime"),f.col("onTime"))
    r6DF = r5DF.dropDuplicates(["lineID","stationID"])    
